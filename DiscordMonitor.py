...
def downloadImage(url):
    ...
    res = requests.get(url,headers=header, proxies=proxys)
    print("call image ok content: ", res.content, " status: ", res.status_code)

    if res.status_code == 200:
        print("will write")
        # open('/fastdata/abc.png', 'xwb').write(res.content)
        f = open('abc.png', 'wb')
        print("open")
        f.write(res.content)
        print("write")
...
class DiscordMonitor(discord.Client):
    ...
    def dingmessage(self, msg):
        ...
        info = requests.post(url=self.dingding, data=message_json, headers=headers)
        print(info.text)
    ...
    async def process_message(self, message: discord.Message, status):
        ...
        print("-------> push_text: ", push_text)
        self.dingmessage(push_text)
